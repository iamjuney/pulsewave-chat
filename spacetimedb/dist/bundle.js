...
function requireRoomMember(ctx, roomId) {
...
const update_profile = spacetimedb.reducer({
	bio: t.string().optional(),
	avatarUrl: t.string().optional()
}, (ctx, { bio, avatarUrl }) => {
	const user = ctx.db.user.identity.find(ctx.sender);
...
const set_typing = spacetimedb.reducer({ typing: t.bool() }, (ctx, { typing }) => {
	const user = ctx.db.user.identity.find(ctx.sender);
	if (!user) throw new SenderError("Unknown user");
	ctx.db.user.identity.update({
		...user,
...
const create_room = spacetimedb.reducer({ name: t.string() }, (ctx, { name }) => {
...
const join_room = spacetimedb.reducer({ roomId: t.u64() }, (ctx, { roomId }) => {
	...
	for (const m of ctx.db.room_member.iter()) {
		if (m.roomId !== roomId) continue;
		if (m.userId.toHexString() === ctx.sender.toHexString()) throw new SenderError("Already a member");
	}
	ctx.db.room_member.insert({
		id: 0n,
		roomId,
...
const leave_room = spacetimedb.reducer({ roomId: t.u64() }, (ctx, { roomId }) => {
	const membership = requireRoomMember(ctx, roomId);
	ctx.db.room_member.id.delete(membership.id);
});
const create_dm = spacetimedb.reducer({ targetIdentity: t.identity() }, (ctx, { targetIdentity }) => {
	if (ctx.sender.toHexString() === targetIdentity.toHexString()) throw new SenderError("Cannot create DM with yourself");
	for (const m of ctx.db.room_member.iter()) {
		if (m.userId.toHexString() !== ctx.sender.toHexString()) continue;
		const rm = ctx.db.room.id.find(m.roomId);
		if (rm && rm.isDm) for (const m2 of ctx.db.room_member.iter()) {
			if (m2.roomId !== m.roomId) continue;
			if (m2.userId.toHexString() === targetIdentity.toHexString()) return;
		}
	}
...
const send_message = spacetimedb.reducer({
	...
	if (!text && !imageUrl) throw new SenderError("Message must have text or image");
...
const edit_message = spacetimedb.reducer({
	...
	if (msg.sender.toHexString() !== ctx.sender.toHexString()) throw new SenderError("Cannot edit another user's message");
...
const delete_message = spacetimedb.reducer({ messageId: t.u64() }, (ctx, { messageId }) => {
	const msg = ctx.db.message.id.find(messageId);
	if (!msg) throw new SenderError("Message not found");
	if (msg.sender.toHexString() !== ctx.sender.toHexString()) throw new SenderError("Cannot delete another user's message");
...
const mark_read = spacetimedb.reducer({
	roomId: t.u64(),
	messageId: t.u64()
}, (ctx, { roomId, messageId }) => {
	const membership = requireRoomMember(ctx, roomId);
	if (messageId > membership.lastReadMessageId) ctx.db.room_member.id.update({
		...membership,
		lastReadMessageId: messageId
	});
});
const add_reaction = spacetimedb.reducer({
	messageId: t.u64(),
	emoji: t.string()
}, (ctx, { messageId, emoji }) => {
	...
	for (const r of ctx.db.reaction.iter()) {
		if (r.messageId !== messageId) continue;
		if (r.sender.toHexString() === ctx.sender.toHexString() && r.emoji === emoji) throw new SenderError("Already reacted with this emoji");
	}
...
const remove_reaction = spacetimedb.reducer({
	messageId: t.u64(),
	emoji: t.string()
}, (ctx, { messageId, emoji }) => {
	for (const r of ctx.db.reaction.iter()) {
		if (r.messageId !== messageId) continue;
		if (r.sender.toHexString() === ctx.sender.toHexString() && r.emoji === emoji) {
			ctx.db.reaction.id.delete(r.id);
			return;
		}
	}
...
const onConnect = spacetimedb.clientConnected((ctx) => {
	...
	if (user) ctx.db.user.identity.update({
		...
		for (const rm of ctx.db.room.iter()) if (rm.name === "general" && !rm.isDm) {
			let isMember = false;
			for (const m of ctx.db.room_member.iter()) {
			...
			if (!isMember) ctx.db.room_member.insert({
				id: 0n,
				roomId: rm.id,
				userId: ctx.sender,
				joinedAt: ctx.timestamp,
...
//#endregion
export { add_reaction, create_dm, create_room, spacetimedb as default, delete_message, edit_message, init, join_room, leave_room, mark_read, onConnect, onDisconnect, remove_reaction, send_message, set_name, set_typing, update_profile };
//# debugId=5346745e-bdca-46ff-a361-7590f5971355
//# sourceMappingURL=data:application/json;charset=utf-8;base64,